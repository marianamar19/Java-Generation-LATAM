...
function construirCardProducto(p) {
    ...
    card.innerHTML = `
        <div class="ed-item-header">
            ${badgeHTML}
            <button class="fav-btn${isFav ? ' active' : ''}" data-product-id="${p.productId}" data-brand="${p.marca}" data-name="${p.nombre}" data-price="${p.precio}" data-nivel="${nivel}" data-vol-label="Presentación" data-tipo="${p.tipo}" aria-label="Añadir a favoritos">
        ...
        <div class="ed-footer">
            ...
            <a href="producto.html" class="ed-cta" data-slug="${p.slug}">Ver producto <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6"/></svg></a>
        </div>
